...
function Chatbot() {
  ...
  const handleSend = () => {
    ...
    // Simulated bot reply (replace with backend later)
    setTimeout(() => {
      setMessages([
        ...newMessages,
        { sender: "bot", text: `🤖 You asked: "${input}". Here's a helpful explanation...` }
      ]);
    }, 1000);
  ...
  return (
    <div className="w-80 bg-white dark:bg-zinc-800 shadow-lg rounded-lg border border-zinc-300 dark:border-zinc-700">
      ...
      <div className="flex border-t border-zinc-300 dark:border-zinc-700">
        ...
        <button
          onClick={handleSend}
          className="px-3 bg-blue-600 text-white text-sm rounded-r-lg hover:bg-blue-700"
        >
          Send
        </button>
      </div>
...
export default function Home() {
  ...
  const handleDownloadTXT = () => {
    ...
    a.href = url;
    a.download = `${subject}-${topic}-notes.txt`;
    a.click();
  ...
  const handleDownloadPDF = () => {
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = `${subject}-${topic}-notes.pdf`;
    a.click();
    URL.revokeObjectURL(url);
  };
    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-zinc-50 font-sans dark:bg-black p-10">
      ...
      {notes && (
        <div className="mt-8 p-4 border rounded-lg bg-white dark:bg-zinc-800 w-full max-w-xl fade-in">
          ...
          <div className="prose prose-zinc dark:prose-invert whitespace-pre-line mb-4">
            {notes}
          </div>
